...
- 2026-04-15: Replaced hardcoded hand gesture recognition in `components/control/controlClient.ts` with template matching. Removed finger-angle/pinch/open-palm, ULT combo, and GIANT hold detection paths while keeping MediaPipe `HandLandmarker`, camera start, socket commit, aim stream, and `requestAnimationFrame` flow. Added `components/control/gestureTemplate.ts` with wrist-origin normalization, wrist-to-middle-MCP 3D scale, and average Euclidean similarity. Added localStorage-backed `saveGesture(gestureName)`, `window.saveGesture`, saved gesture status, and Play-mode threshold matching at error < 0.3. Added `handOverlay` canvas over camera preview in `/control` and embedded `/display` control UI with red landmarks and translucent white connectors.
- 2026-04-15: Verification: `npx tsc --noEmit` passed; `npm run build` passed. Playwright client run failed because package `playwright` is not installed (`ERR_MODULE_NOT_FOUND` from `web_game_playwright_client.js`), so camera overlay visual validation could not be captured in this environment.

- 2026-04-16: Upgraded gesture recording from single-frame templates to 0.5s motion templates. Added `components/control/skillCatalog.ts` reading all available skills from `components/display/battlefield/config.ts` / `SKILL_GROUPS`, rendered skill dropdowns in `/control` and embedded `/display` controls, replaced Save with Start Record, added full-screen 3-2-1/REC/SAVED countdown overlay, and stored motion frames with normalized hand landmarks plus normalized arm pose direction vectors. Matching now uses recent live motion window and combines hand + arm error for complex gestures while retaining legacy single-frame localStorage fallback.
- 2026-04-16: Verification: `npx tsc --noEmit` passed; `npm run build` passed. Playwright client still cannot run because package `playwright` is not installed (`ERR_MODULE_NOT_FOUND`). Next dev server started successfully at `http://localhost:3000`.

- 2026-04-17: Fixed post-record false activation / non-recast issue. After saving a motion template, control now clears the live motion buffer, resets the committed gesture to IDLE, and requires the user to release/leave the recorded pose (`TEMPLATE_RELEASE_THRESHOLD`) before template matching can fire. Added `activeMatchedGesture` re-arm logic so the same saved gesture can cast again after leaving and re-entering the match zone, while still avoiding spam while holding the same pose. This addresses record-triggered skill cooldown/qi drain causing later FAN/SPHERE/SPIN matches to be detected but not cast. Verification: `npx tsc --noEmit` passed; `npm run build` passed.

- 2026-04-17: Relaxed the post-record release gate because it blocked casts when the player stayed near the recorded pose. Replaced strict release threshold with a short `POST_RECORD_SUPPRESS_MS` window and `MATCH_RECAST_MS` resend cadence. Matching now resumes automatically after ~700ms and can resend the same matched gesture periodically so display-side cooldown/qi no longer makes a successfully recognized FAN/SPHERE/SPIN look stuck. Verification: `npx tsc --noEmit` passed; `npm run build` passed.

- 2026-04-17: Suppressed the noisy MediaPipe/TFLite XNNPACK delegate INFO line that Next dev overlay reported at `detectForVideo`. Added a scoped console filter in `controlClient.ts` that drops only `INFO: Created TensorFlow Lite XNNPACK delegate for CPU.` from `console.error/warn/info`, with cleanup restoring original console methods. Verification: `npx tsc --noEmit` passed; `npm run build` passed.

- 2026-04-17: Display UI polish: compacted the HP/QI HUD panels (smaller width, padding, typography, bars, badges), moved the sword selector from the standalone bottom-left floating picker into the control/settings panel, and raised the camera preview + hand overlay from the bottom edge. Verification: `npx tsc --noEmit` passed; `npm run build` passed. Playwright visual check still blocked by missing local `playwright` package.

- 2026-04-17: Restored GIANT / Tam Nhẫn Kiếm Chỉ as a template-matched hold skill instead of an instant cast. When the saved GIANT template matches, control now sends `GIANT_CHARGE` once, requires the pose to stay matched for 3000ms, then sends `GIANT` to fire. During the hold phase, control compares the current hand+arm pose with the final frame of the saved GIANT template so the player can hold a pose instead of replaying the 0.5s recording repeatedly. If the pose is lost for more than a short jitter grace window, control sends `GIANT_CANCEL`; after firing, the player must release/change pose before another charge can start. GIANT charge rings now spawn every 1.0s so the 3s hold lines up with three rings. Verification: `npx tsc --noEmit` passed; `npm run build` passed; `git diff --check` passed; Playwright client still blocked by missing local `playwright` package (`ERR_MODULE_NOT_FOUND`).

- 2026-04-17: Smoothed GIANT / Tam Nhẫn Kiếm Chỉ firing after the 3s hold. Synced combat hit count with VFX rings by setting GIANT to 30 shots (3 rings x 10 swords), added `hitDelaySec` so damage/hurt events happen near projectile impact instead of immediately after launch, reduced per-hit hitstop for GIANT so the sword stream does not stutter, and made heavy reactions deterministic at ring boundaries/final sword. `GiantChargeRender` now derives `countPerRing` from `SKILLS.GIANT.meta.shots`. `HitRender` has a GIANT-specific lightweight impact reaction, and `StickFighter.playHit` uses shorter GIANT hit timing so repeated sword impacts read as successive hits instead of constantly restarting a long hurt animation. Model clip fade now scales down for short-duration hit clips so GIANT micro-hits remain visible. Verification: `npx tsc --noEmit` passed; `npm run build` passed; `git diff --check` passed; Playwright client still blocked by missing local `playwright` package (`ERR_MODULE_NOT_FOUND`).

- 2026-04-17: Fixed GIANT ring re-spawn bug during firing. Root cause: after a ring finished launching and was removed, `giantCharges` still behaved like an active charge, so `update.ts` saw `rings.length < maxRings` and added a fresh ring while lower rings were firing. `fireGiantFromStackedRings` now marks the charge state as `firing` and disables `nextRingAt`; `giantAddRing` and the giant charge update loop both skip ring creation while firing. Result: once a ring has fired all swords, it disappears and does not come back. Verification: `npx tsc --noEmit` passed; `npm run build` passed; `git diff --check` passed; Playwright client still blocked by missing local `playwright` package (`ERR_MODULE_NOT_FOUND`).

- 2026-04-17: Reduced MediaPipe CPU load in control. Root cause: `HandLandmarker` and `PoseLandmarker` were created without `baseOptions.delegate`, so Tasks Vision used the CPU/XNNPACK path, and hand detection was running on every `requestAnimationFrame`. Added GPU delegate creation for both hand and pose models with CPU fallback if GPU/WebGL delegate fails; moved model URLs into constants; throttled hand detection to 24Hz and pose detection/sending to 18Hz; debug mode no longer bypasses pose throttling. Verification: `npx tsc --noEmit` passed; `npm run build` passed; `git diff --check` passed; Playwright client still blocked by missing local `playwright` package (`ERR_MODULE_NOT_FOUND`).
